function orientationChangeHandler(e) {
    disableScrollOrSwipe(), setTimeout(
        function() {
            $(window).trigger(
                "resize")
        }, 500)
...
function initVariablesAfterShowContainer() {
    fireworkCenterX = .5 *
        fireworkArray[0].offsetWidth,
        fireworkCenterY = .5 *
        fireworkArray[0].offsetHeight,
        fireworkOneRadiusDistance = (
            fireworkCenterY -
            fireworkDotRadius) /
        fireworkRowNumber,
        fireworkOneRotationAngle = 2 *
        Math.PI / fireworkColumnNumber
}

function resetVariables() {
    pageVerticalPosition = 0,
        canAnimatePlantInformation = !0,
        canAnimateBuildingInformation = !
        0, 0 == isFishStillAnimating &&
        (canAnimateFishInformation = !0),
        0 == isCrabStillAnimating && (
            canAnimateCrabInformation = !
            0), 0 ==
        isTurtleStillAnimating && (
...
function resetFunctions() {
    positionPlants(), positionBuildings(),
        0 == isFishStillAnimating &&
        positionSeaAnimals(fishArray,
            numberOfFishInEachRowArray,
            150, 100), 0 ==
        isCrabStillAnimating &&
        positionSeaAnimals(crabArray,
            numberOfCrabInEachRowArray,
            150, 100), 0 ==
        isTurtleStillAnimating &&
        positionSeaAnimals(turtleArray,
...
function handleStart(e) {
    touchStartX = e.targetTouches[0].pageX,
        pageVerticalPositionOnTouch =
        pageVerticalPosition
}

function handleMove(e) {
    e.preventDefault(), touchCurrentX =
        e.targetTouches[0].pageX, 1 ==
        canScrollOrSwipe && (
...
function handleEnd(e) {
    e.preventDefault(), touchEndX = e.changedTouches[
        0].pageX
}
...
function shiftUpHorizontalLayersAfterEverythingLoaded() {
    for (var e = 0; e <
        layerHorizontalArray.length; e++
    ) $(layerHorizontalArray[e]).stop()
        .animate({
            top: "0px"
        }, 1e3, function() {
...
function setBannersContainerVerticalPosition() {
    bannersContainerDiv.style.bottom =
        containerDiv.offsetHeight +
        "px"
}
...
function setLayerSpeed() {
    for (; layerHorizontalSpeedArray.length >
        0;) layerHorizontalSpeedArray.pop();
    for (; layerVerticalSpeedArray.length >
        0;) layerVerticalSpeedArray.pop();
    for (var e = 0; e <
        layerHorizontalArray.length; e++
    ) {
        var t = (layerHorizontalArray[e]
            .offsetWidth -
            containerDiv.offsetWidth
        ) / (layerHorizontalArray[
                layerHorizontalArray
        ...
        layerHorizontalSpeedArray.push(
            t)
    }
    for (var e = 0; e <
        layerVerticalArray.length; e++) {
        var i = (layerVerticalArray[e].offsetHeight -
            containerDiv.offsetHeight
        ) / (layerVerticalArray[
                layerVerticalArray.length -
                1].offsetHeight -
...
function detectPageVerticalPosition() {
    previousPageVerticalPosition =
        ...
        pageVerticalPosition =
        "internet explorer" ==
        browserName ? document.documentElement
...
function moveLayers() {
    if (setLayersMovement(),
        ...
        for (var e = 0; e <
            layerHorizontalArray.length; e++
        ) layerHorizontalArray[e].style
            .left = -1 *
            layerHorizontalSpeedArray[e] *
...
function setRobbyLeftAndRightEdge() {
    var e = 65;
    robbyRightEdge = .5 * (containerDiv
        .offsetWidth + robbyDiv.offsetWidth
    ) - e, robbyLeftEdge = .5 * (
        containerDiv.offsetWidth -
        robbyDiv.offsetWidth) + e
}
...
function positionVerticalLayersToHaveSameTopPosition() {
    for (var e = 0; e <
        ...
        layerVerticalArray[e].style.bottom =
        containerDiv.offsetHeight -
        layerVerticalArray[e].offsetHeight +
        "px"
}
...
function shiftUpDownHorizontalLayers() {
    (previousPageVerticalPosition <
        ...
        robbyLeftEdge ||
        previousPageVerticalPosition >
        sea1Div.offsetLeft + sea1Div.offsetWidth -
        robbyRightEdge) &&
    pageVerticalPosition >= sea1Div.offsetLeft -
        robbyLeftEdge &&
        pageVerticalPosition <= sea1Div
        .offsetLeft + sea1Div.offsetWidth -
        robbyRightEdge && (
            isRobbySwimming = !0,
    ...
    if ("horizontal" == layersMovement) {
        for (var e = 0; e <
            ...
            var t =
                ...
                .offsetTop -
                shiftUpDownLayerHorizontalIncrement; -
            shiftUpLayerHorizontalDistance
                >=
                t ? (t = -
                    shiftUpLayerHorizontalDistance,
...
function shiftRobbyToGroundLevel() {
    $(robbyContainerDiv).stop().animate({
        bottom: containerDiv.offsetHeight -
            groundAndGrassContainer1Div
            .offsetTop + "px"
    }, 300, function() {})
}
...
function checkRobbyJumpFallSwim() {
    if ("horizontal" == layersMovement)
        if (1 == isRobbySwimming) 1 ==
            isRobbyBelowSeaLevel &&
            robbySwimUp();
        else
            for (var e = 0; e <
                elevationArray.length; e++
            ) robbyJumpUp(e), robbyFall(
                e)
}
...
function setRobbyJumpUpFrame() {
    clearShiftRobbyFrameTimer(),
        isRobbyJumping = !0,
        robbyFramesDiv.style.left = -1 *
        robbyStartJumpFrame *
        robbyOneFrameWidth + "px"
}
...
function disableIsRobbyJumpingAndFalling() {
    isRobbyJumping = !1, isRobbyFalling = !
        1
}

function robbySwimUp() {
    if (getSwimUpHeight(), swimUpHeight >
        0) {
        var e = seaFloorDiv.offsetHeight +
            swimUpHeight + "px",
            t = 3 * swimUpHeight,
            i = 6 * swimUpHeight;
        $(robbyContainerDiv).stop().animate({
...
function robbySwimDown(e) {
    $(robbyContainerDiv).stop().animate({
        ...
        robbyContainerDiv.offsetHeight <=
        containerDiv.offsetHeight -
...
function getSwimUpHeight() {
    swimUpHeight = Math.abs(
        deltaPageVerticalPosition);
    var e = sea1Div.offsetHeight -
        robbyDiv.offsetHeight;
    swimUpHeight > e && (swimUpHeight =
        e)
}
...
function positionVerticalLayersHorizontally() {
    for (var e = 0; e <
        ...
        layerHorizontalArray[e].offsetWidth -
        containerDiv.offsetWidth + "px"
}

function positionBalloonAndRobbyContainerHorizontally() {
    var e = pageVerticalPosition *
        layerHorizontalSpeedArray[
            layerHorizontalSpeedArray.length -
            1] - (layerHorizontalArray[
                layerHorizontalArray.length -
                1].offsetWidth -
            containerDiv.offsetWidth),
        t = 250,
        i = 50;
    robbyMaxHorizontalDistance = .5 *
        containerDiv.offsetWidth + t;
    var n = .5 * containerDiv.offsetWidth +
        e;
    n >= robbyMaxHorizontalDistance &&
        (n = robbyMaxHorizontalDistance);
    var a = .5 * containerDiv.offsetWidth +
        i,
        o = .5 * (containerDiv.offsetWidth -
            balloonDiv.offsetWidth) + e;
    o >= a && (o = a), "vertical" ==
        layersMovement ? (balloonDiv.style
            .left = o + "px",
        ...
        layersMovement ? (
            robbyContainerDiv.style.left =
            n + pageVerticalPosition -
            (pageDiv.offsetHeight -
                containerDiv.offsetHeight -
                distanceBetweenRobbyAndBalloon
            ) + "px", balloonDiv.style.left =
            o + "px") : (balloonDiv.style
            .left =
            layerHorizontalArray[
                layerHorizontalArray.length -
                1].offsetLeft +
            layerHorizontalArray[
                ...
                1].offsetWidth - .5 * (
                containerDiv.offsetWidth +
                balloonDiv.offsetWidth) +
            "px", robbyContainerDiv.style
            .left = "50%")
}

function positionBalloonVertically() {
    balloonDiv.style.bottom = .2 *
        containerDiv.offsetHeight +
        "px"
}
...
function setLayersMovement() {
    layersMovement =
        ...
        containerDiv.offsetWidth ?
        "horizontal" :
        pageVerticalPosition >= pageDiv
        .offsetHeight - containerDiv.offsetHeight -
        distanceBetweenRobbyAndBalloon &&
        pageVerticalPosition < pageDiv.offsetHeight -
        containerDiv.offsetHeight ?
        "not moving 1" :
        pageVerticalPosition >= pageDiv
        .offsetHeight - containerDiv.offsetHeight ?
        "not moving 2" : "vertical"
}

function orientRobby() {
    deltaPageVerticalPosition > 0 && (
            robbyFramesDiv.style.top =
            "0px", robbyEyesCloseDiv.style
            .left = "90px"), 0 >
        deltaPageVerticalPosition && (
            robbyFramesDiv.style.top =
            "-200px", robbyEyesCloseDiv
            .style.left = "55px")
}

function storeDivs() {
    for (var e = document.getElementsByTagName(
            "div"), t = 0; t < e.length; t++)
        "fish" == e[t].getAttribute(
            "class") && fishArray.push(
            e[t]), "fish-eyes" == e[t].getAttribute(
            "class") && fishEyeArray.push(
            e[t]), "crab" == e[t].getAttribute(
            "class") && crabArray.push(
            e[t]), "crab-eyes" == e[t].getAttribute(
            "class") && crabEyeArray.push(
            e[t]), "turtle" == e[t].getAttribute(
            "class") && turtleArray.push(
            e[t]), "turtle-eyes" == e[t]
        .getAttribute("class") &&
        turtleEyeArray.push(e[t]), (
            "nba-board-blue" == e[t].getAttribute(
                "class") ||
            "nba-board-red" == e[t].getAttribute(
                "class")) &&
        nbaBoardArray.push(e[t]),
        "elevation" == e[t].getAttribute(
            "class") && elevationArray.push(
            e[t]), "plant" == e[t].getAttribute(
            "class") && plantArray.push(
            e[t]), "building" == e[t].getAttribute(
            "class") && buildingArray.push(
            e[t]), (
            "building-enemy-face-a-eyes" ==
            e[t].getAttribute("class") ||
            "building-enemy-face-b-eyes" ==
            e[t].getAttribute("class")) &&
        buildingEnemyFaceEyeArray.push(
            e[t]), (
            "building-leg-frame-a" == e[
                t].getAttribute("class") ||
            "building-leg-frame-b" == e[
                t].getAttribute("class")
        ) && buildingLegFrameArray.push(
            e[t]), (
            "building-leg-container-a" ==
            e[t].getAttribute("class") ||
            "building-leg-container-b" ==
            e[t].getAttribute("class")) &&
        buildingLegContainerArray.push(
            e[t]),
        ...
        e[t].getAttribute("class") &&
        experienceTextContainerArray.push(
            e[t]),
        "chain-block-and-string-container" ==
        e[t].getAttribute("class") &&
        chainBlockAndStringContainerArray
        .push(e[t]), "layer-horizontal" ==
        e[t].getAttribute("class") &&
        layerHorizontalArray.push(e[t]),
        "layer-vertical" == e[t].getAttribute(
            "class") &&
        layerVerticalArray.push(e[t]),
        ("algae-a" == e[t].getAttribute(
                "class") || "algae-b" ==
            e[t].getAttribute("class") ||
            "title-skills-class" == e[t]
            .getAttribute("class")) &&
        seaFloorFrontObjectArray.push(e[
            t]), ("coral" == e[t].getAttribute(
                "class") || "coral-big" ==
            e[t].getAttribute("class")) &&
        seaFloorBackObjectArray.push(e[
            t]), "squid-hand-close" ==
        e[t].getAttribute("class") &&
        squidHandCloseArray.push(e[t]),
        "squid-hand-open" == e[t].getAttribute(
            "class") &&
        squidHandOpenArray.push(e[t]),
        "firework" == e[t].getAttribute(
            "class") && fireworkArray.push(
            e[t])
}

function animatePlants() {
    for (var e = 0; e < plantArray.length; e++)
        $(plantArray[e]).stop().delay(
            300 * e).animate({
            top: [
...
function positionPlants() {
    for (var e = 0; e < plantArray.length; e++)
        ...
        "100%" : plantArray[e].style.top =
        plantTargetTopObjectArray[e].offsetTop +
        "px"
}

function animateBuildings() {
    clearInterval(buildingLegsTimer),
        buildingLegsTimer = setInterval(
            function() {
                animateBuildingsLegs()
            }, 200);
    for (var e = 0; e < buildingArray.length; e++)
        $(buildingArray[e]).stop().delay(
            300 * e).animate({
            left: [
...
function animateBuildingsLegs() {
    buildingCounter += 1;
    for (var e = 0; e < buildingArray.length; e++) {
        if (buildingArray[buildingArray
                .length - 1].offsetLeft ==
            buildingTargetLeftArray[
                buildingTargetLeftArray
                .length - 1]) return buildingLegFrameArray[
                buildingLegFrameArray
                .length - 1].style.left =
            "0px", clearInterval(
                buildingLegsTimer),
            void(buildingCounter =
                0);
        buildingArray[e].offsetLeft >
            buildingTargetLeftArray[e] &&
            buildingArray[e].offsetLeft <
            buildingEarlyPositionArray[
                e] ?
            buildingLegFrameArray[e].style
            .left = -1 *
            buildingLegContainerArray[e]
            .offsetWidth * (
                buildingCounter % 2) +
            "px" :
            buildingLegFrameArray[e].style
...
function blinkBuildings() {
    if (pageVerticalPosition + .5 *
        containerDiv.offsetWidth <
        about2ContainerDiv.offsetLeft ||
        pageVerticalPosition + .5 *
        containerDiv.offsetWidth >
        about2ContainerDiv.offsetLeft +
        about2ContainerDiv.offsetWidth)
        clearInterval(
            buildingBlinkTimer);
    else {
        var e = Math.floor(Math.random() *
            buildingArray.length);
        $(buildingEnemyFaceEyeArray[e])
            .fadeTo(0, 1), $(
                buildingEnemyFaceEyeArray[
                    e]).stop().delay(
                300).animate({
                opacity: 0
            }, 0, function() {})
...
function positionBuildings() {
    for (var e = 0; e < buildingArray.length; e++)
        buildingArray[e].style.left =
        buildingEarlyPositionArray[e] +
        "px"
}

function nbaBoardsJump() {
    for (var e = 0; e < nbaBoardArray.length; e++)
        $(nbaBoardArray[e]).stop().delay(
            100 * e).animate({
            bottom: [200,
                "easeOutCubic"
            ]
...
function nbaBoardsFall() {
    $(nbaBoardArray[nbaBoardsCounter]).stop()
        .animate({
            bottom: [0,
                "easeInCubic"
            ]
        ...
        nbaBoardsCounter += 1,
        nbaBoardsCounter >=
        nbaBoardArray.length && (
            nbaBoardsCounter = 0)
}

function positionNbaBoard() {
    for (var e = 0; e < nbaBoardArray.length; e++)
        nbaBoardArray[e].style.bottom =
        "0px"
}
...
function nbaPlayerRun() {
    clearInterval(nbaPlayerTimer),
        nbaPlayerTimer = setInterval(
            function() {
                animateNbaPlayerRun()
            }, 200), $(
            nbaPlayerContainerDiv).stop()
        .animate({
            left: "690px"
        }, 1e3, function() {
            nbaPlayerJump()
...
function nbaPlayerFall() {
    shiftNbaPlayerFrame(3), shakeRim(),
        bounceBall(), nbaBoardsJump(),
...
function shiftNbaPlayerFrame(e) {
    nbaPlayerFrameDiv.style.left = -300 *
        e + "px"
}
...
function positionNbaPlayerContainer() {
    nbaPlayerContainerDiv.style.left =
        "1400px", nbaPlayerContainerDiv
        .style.bottom = "0px"
}
...
function blinkNbaPlayer() {
    pageVerticalPosition + .5 *
        ...
        containerDiv.offsetWidth >
        about3ContainerDiv.offsetLeft +
        about3ContainerDiv.offsetWidth ?
        clearInterval(
            blinkNbaPlayerTimer) : ($(
                nbaPlayerEyesDiv).fadeTo(
                0, 1), $(
                nbaPlayerEyesDiv).stop()
            .delay(300).animate({
                opacity: 0
            }, 0, function() {}))
}
...
function stopAllNbaAnimation() {
    ...
    $(nbaPlayerContainerDiv).stop(!0, !
        1)
}

function positionSeaAnimals(e, t, i, n) {
    for (var a = e, o = t, r = i, l = n,
            s = 0, c = 0; c < o.length; c++)
        for (var f = 0; f < o[c]; f++) a[
                s].style.left =
            seaAnimalSwimDistance + f *
            r + "px", a[s].style.top =
            c * l + "px", s += 1
}
...
function animateSeaAnimals(e) {
    var t = e;
    t == fishArray && (
            isFishStillAnimating = !0),
        t == crabArray && (
            isCrabStillAnimating = !0),
        t == turtleArray && (
            isTurtleStillAnimating = !0
        );
    for (var i = 0; i < t.length; i++) $(
            t[i]).stop().delay(100 * i)
        .animate({
            left: [t[i].offsetLeft -
            ...
            disableIsSeaAnimalStillAnimating
                (t)
        })
}

function disableIsSeaAnimalStillAnimating(
    e) {
    var t = e;
    t == fishArray && (
        fishAnimateNumber >= t.length -
        1 ? (isFishStillAnimating = !
            1, fishAnimateNumber =
            0) : fishAnimateNumber +=
        1), t == crabArray && (
        crabAnimateNumber >= t.length -
        1 ? (isCrabStillAnimating = !
            1, crabAnimateNumber =
            0) : crabAnimateNumber +=
        1), t == turtleArray && (
        turtleAnimateNumber >= t.length -
        1 ? (isTurtleStillAnimating = !
            1, turtleAnimateNumber =
            0) :
        turtleAnimateNumber += 1)
}

function animateRobbyRunSwim() {
    1 == canAnimateRobbyRunSwim && 0 ==
        isRobbyJumping && 0 ==
        isRobbyFalling && "vertical" !=
        layersMovement && (
...
function shiftRobbyFrame() {
    ...
    if (1 == isRobbySwimming && 1 ==
        ...
        robbyFramesDiv.style.left = -1 *
        robbyOneFrameWidth * (
            robbyStartFrame + counter) +
        "px", robbyStartFrame + counter +
        switcher > robbyStopFrame && (
            switcher = -1 * switcher),
        robbyStartFrame + counter +
        switcher == robbyStartFrame &&
        (
...
function animateChainBlockAndStringContainer(
    e) {
    $(chainBlockAndStringContainerArray[
        e]).stop().animate({
        bottom: [
...
function positionExperienceTextContainer() {
    for (var e = 0; e <
        experienceTextContainerArray.length; e++
    ) 0 == e && (
            canAnimateBossInformation =
            canAnimateRobotInformation),
        1 == e && (
...
function animateExperienceTextContainer(
    e) {
    $(experienceTextContainerArray[e]).stop()
        .animate({
            bottom: [
                experienceTextContainerDistanceFromFloor,
                "easeOutCubic"
...
function positionExperience1Elements() {
    robotDiv.style.left =
        ...
        "px", $(
            piechartAolTextGraphic1Div)
        .fadeTo(0, 0), $(
            piechartAolTextGraphic2Div)
        .fadeTo(0, 0)
}
...
function positionExperience2Elements() {
    squidDiv.style.left =
        ...
        "px", $(
            piechartIncognitoTextGraphic1Div
        ).fadeTo(0, 0), $(
            piechartIncognitoTextGraphic2Div
        ).fadeTo(0, 0), $(
            piechartIncognitoTextAnimation1Div
        ).fadeTo(0, 0), $(
            piechartIncognitoTextAnimation2Div
        ).fadeTo(0, 0), $(
            piechartIncognitoTextCode1Div
        ).fadeTo(0, 0), $(
            piechartIncognitoTextCode2Div
        ).fadeTo(0, 0),
        "internet explorer" ==
        browserName && browserVersion <=
        8 || $(
            piechartIncognitoFrontDiv).fadeTo(
            0, 0)
}
...
function positionExperience3Elements() {
    alienDiv.style.left =
        ...
        "px", $(
            piechartFoxnewsTextGraphic1Div
        ).fadeTo(0, 0), $(
            piechartFoxnewsTextGraphic2Div
        ).fadeTo(0, 0), $(
            piechartFoxnewsTextAnimation1Div
        ).fadeTo(0, 0), $(
            piechartFoxnewsTextAnimation2Div
        ).fadeTo(0, 0), $(
            piechartFoxnewsTextCode1Div
        ).fadeTo(0, 0), $(
            piechartFoxnewsTextCode2Div
        ).fadeTo(0, 0),
        "internet explorer" ==
        browserName && browserVersion <=
        8 || $(piechartFoxnewsFrontDiv)
        .fadeTo(0, 0)
}
            ...
            for (var e = 0; e <
                ...
                landInformationContainerArray[
                    ...
                    (0 ==
                        ...
                        animateRobotHands() :
                        (animateRobot(),
                            animateExperienceTextContainer(
                                0),
                            animateChainBlockAndStringContainer(
                                0),
                            canAnimateRobotInformation = !
                            1)),
                    landInformationContainerArray[
...
function changeRobotHands() {
    if (changeRobotHandsCounter >=
        robotHandChildrenLength)
        ...
        for (var e = 0; robotHandChildrenLength >
            e; e++) e ==
            changeRobotHandsCounter ?
            setRobotHandsToOpaque(e) :
            setRobotHandsToTransparent(
                e);
...
function setRobotHandsToOpaque(e) {
    robotHandLeftDiv.children[e].style.opacity =
        1, robotHandLeftDiv.children[e]
        .style.filter =
        "alpha(opacity=100)",
        robotHandRightDiv.children[e].style
        .opacity = 1, robotHandRightDiv
        .children[e].style.filter =
        "alpha(opacity=100)"
}

function setRobotHandsToTransparent(e) {
    robotHandLeftDiv.children[e].style.opacity =
        0, robotHandLeftDiv.children[e]
        .style.filter =
        "alpha(opacity=0)",
        robotHandRightDiv.children[e].style
        .opacity = 0, robotHandRightDiv
        .children[e].style.filter =
        "alpha(opacity=0)"
}
...
function openAndCloseSquidHands() {
    openAndCloseSquidHandsCounter >= 8 ?
...
function openSquidHands() {
    for (var e = 0; e <
        squidHandOpenArray.length; e++)
        squidHandOpenArray[e].style.opacity =
        1, squidHandOpenArray[e].style.filter =
        "alpha(opacity=100)";
    for (var e = 0; e <
        squidHandCloseArray.length; e++
    ) squidHandCloseArray[e].style.opacity =
        0, squidHandCloseArray[e].style
...
function closeSquidHands() {
    for (var e = 0; e <
        squidHandOpenArray.length; e++)
        squidHandOpenArray[e].style.opacity =
        0, squidHandOpenArray[e].style.filter =
        "alpha(opacity=0)";
    for (var e = 0; e <
        squidHandCloseArray.length; e++
    ) squidHandCloseArray[e].style.opacity =
        1, squidHandCloseArray[e].style
...
function rotateAlienHands() {
    alienSteerPreviousAngle =
        alienSteerAngle,
        alienSteerAngle +=
        alienSteerAngleIncrement,
        alienSteerAngle >
        alienSteerPreviousAngle ?
        alienSteerAngle >
        alienSteerAngleLimit && (
            alienSteerAngleIncrement = -
            1 *
            alienSteerAngleIncrement,
        ...
        alienSteerAngle && (
            ...
            "rotate(0deg)",
            alienSteerDiv.style.OTransform =
            "rotate(0deg)",
            alienSteerDiv.style.msTransform =
            "rotate(0deg)",
            alienSteerDiv.style.transform =
            "rotate(0deg)") : (
            alienSteerDiv.style.webkitTransform =
            "rotate(" + alienSteerAngle +
            "deg)", alienSteerDiv.style
            .MozTransform = "rotate(" +
            alienSteerAngle + "deg)",
            alienSteerDiv.style.OTransform =
            "rotate(" + alienSteerAngle +
...
function animatePiechartIncognitoFront() {
    "internet explorer" == browserName
        ...
        $(piechartIncognitoFrontDiv).stop()
        .animate({
            opacity: 1
        }, 500, function() {
            animatePiechartIncognitoText
...
function animatePiechartAolTextGraphic() {
    $(piechartAolTextGraphic1Div).stop()
        .animate({
            opacity: 1
        }, 1e3, function() {}), $(
            piechartAolTextGraphic2Div)
...
function animatePiechartIncognitoTextCode() {
    $(piechartIncognitoTextCode1Div).stop()
        .animate({
            opacity: 1
        }, 1e3, function() {}), $(
            piechartIncognitoTextCode2Div
...
function animatePiechartFoxnewsTextCode() {
    $(piechartFoxnewsTextCode1Div).stop()
        .animate({
            opacity: 1
        }, 1e3, function() {}), $(
            piechartFoxnewsTextCode2Div
...
function animatePiechartFoxnewsTextGraphic() {
    $(piechartFoxnewsTextGraphic1Div).stop()
        .delay(600).animate({
            opacity: 1
        }, 1e3, function() {}), $(
            piechartFoxnewsTextGraphic2Div
...
function animateBubble() {
    var e = robbyContainerDiv.offsetTop -
        (sea1Div.offsetTop -
            shiftUpLayerHorizontalDistance
        );
    positionBubble(e), showBubble(), $(
...
function positionBubble(e) {
    bubbleDiv.style.left =
        pageVerticalPosition + .5 *
        containerDiv.offsetWidth -
        sea1Div.offsetLeft + "px",
        bubbleDiv.style.top = e + "px"
}

function blinkSeaAnimals(e) {
    for (var t = e, i = new Array, n =
            Math.ceil(5 * Math.random()),
            a = 0; n > a; a++) {
        var o = Math.floor(Math.random() *
            e.length);
        i.push(t[o])
    }
...
function positionSocialContainer() {
    1 == canAnimateSocialContainer ? (
            ...
            socialContainerDiv.style.top =
            "80%") : socialContainerDiv
        .style.top = "0px"
}

function animateSocialContainer() {
    1 == canAnimateSocialContainer && (
        $(socialContainerDiv).stop()
        .animate({
            top: [0,
                "easeOutCubic"
            ]
...
function setSocialContainerOpacity(e) {
    e > 1 && (e = 1), 0 > e && (e = 0);
    for (var t = $(socialContainerDiv).children()
            .length, i = 0; t > i; i++)
        $(socialContainerDiv.children[i])
        .fadeTo(0, e);
    for (var n = $(socialContainerDiv.children[
                1]).children().length,
            i = 0; n > i; i++) $(
        socialContainerDiv.children[
            1].children[i]).fadeTo(
        0, e)
}

function happyRobby() {
    0 == isRobbyHappy && (clearInterval(
            happyRobbyTimer),
        happyRobbyTimer =
        setInterval(function() {
            robbyHandsUp()
...
function robbyHandsUp() {
    robbyFramesDiv.style.left =
        "-1600px", setTimeout(function() {
            setRobbyStaticFrame()
        }, 1e3)
}

function positionSplashContainer() {
    splashContainerDiv.style.left = .5 *
        (containerDiv.offsetWidth -
            splashContainerDiv.offsetWidth
        ) + "px"
}
...
function positionRobbyContainerVertically() {
    1 ==
        isPreloadShiftUpAnimationFinish &&
        ($(robbyContainerDiv).stop(!0, !
                1), setRobbyStaticFrame(),
            1 == isRobbySwimming ?
            positionRobbyAtSeaFloorLevel() :
            (
                checkElevationNumberBelowRobby(),
                null !=
                elevationNumberBelowRobby ?
                robbyContainerDiv.style
                .bottom = containerDiv.offsetHeight -
                elevationArray[
                    elevationNumberBelowRobby
                ].offsetTop + "px" :
...
function positionRobbyAtGroundLevel() {
    robbyContainerDiv.style.bottom = .2 *
        containerDiv.offsetHeight +
        "px"
}
...
function positionSeaFloorObjectsVertically() {
    for (var e = 0; e <
        seaFloorFrontObjectArray.length; e++
    ) seaFloorFrontObjectArray[e].offsetHeight >
        sea1Div.offsetHeight ?
        seaFloorFrontObjectArray[e].style
        .bottom = -1 * (
            seaFloorFrontObjectArray[e]
            .offsetHeight - sea1Div.offsetHeight
        ) + "px" :
        seaFloorFrontObjectArray[e].style
        .bottom = "0px";
    for (var e = 0; e <
        seaFloorBackObjectArray.length; e++
    ) seaFloorBackObjectArray[e].offsetHeight >
        sea1Div.offsetHeight ?
        seaFloorBackObjectArray[e].style
        .bottom = -.7 * containerDiv.offsetHeight -
        (seaFloorBackObjectArray[e].offsetHeight -
            sea1Div.offsetHeight) +
        "px" : seaFloorBackObjectArray[
            e].style.bottom = "-70%"
}
...
function hideScrollOrSwipeTextContainer() {
    1 ==
        canHideScrollOrSwipeTextContainer &&
        (clearInterval(
                scrollOrSwipeTextContainerTimer
            ),
...
function fadeOutScrollOrSwipeTextContainer() {
    $(scrollOrSwipeTextContainer1Div).fadeTo(
        0, 0), $(
        scrollOrSwipeTextContainer2Div
    ).fadeTo(0, 0)
}
...
function positionContactConfirmationContainer() {
    for (var e = 0; e <
        contactConfirmationContainerArray
        .length; e++) "not moving 1" ==
        layersMovement ||
        "not moving 2" ==
        layersMovement ?
        contactConfirmationContainerArray[
            e].style.left =
        robbyContainerDiv.offsetLeft +
        "px" :
        contactConfirmationContainerArray[
            e].style.left =
        ...
        contactConfirmationContainerArray[
            e].style.top = .8 *
        containerDiv.offsetHeight - 370 +
        "px"
}

function hideContactConfirmationContainer() {
    if (1 ==
        isContactConfirmationContainerVisible
    ) {
        for (var e = 0; e <
            contactConfirmationContainerArray
            .length; e++)
...
function showContactConfirmationContainer(
    e) {
    for (var t = $(
                contactConfirmationContainerArray[
                    e]).children().children()
...
function createFireworkSvg() {
    if (!("internet explorer" ==
            ...
            browserVersion <= 8))
        for (var e = 0; e <
            fireworkArray.length; e++) {
            var t = document.createElementNS(
                "http://www.w3.org/2000/svg",
                "svg");
            t.setAttribute("version",
                    "1.2"), t.setAttribute(
                    "baseProfile",
                    "tiny"), t.setAttribute(
                    "width", "100%"), t
...
function drawOneLayerOfFirework() {
    if (fireworkRowNumber >
        fireworkLayerNumber) {
        fireworkLayerNumber += 1;
        for (var e = 0; fireworkColumnNumber >
            e; e++) {
            var t = document.createElementNS(
                "http://www.w3.org/2000/svg",
                "circle");
            t.setAttribute("cx", String(
                    fireworkCenterX +
                    Math.cos(e *
...
var contentDiv = document.getElementById(
        "content"),
    pageDiv = document.getElementById(
        "page"),
    robbyContainerDiv = document.getElementById(
        "robby-container"),
    robbyDiv = document.getElementById(
        "robby"),
    robbyFramesDiv = document.getElementById(
        "robby-slides"),
    robbyEyesCloseDiv = document.getElementById(
        "robby-eyes-close"),
    blinkRobbyEyesTimer,
    bannersContainerDiv = document.getElementById(
        "banners-container"),
    isPreloadShiftUpAnimationFinish = !
    1,
    canFinishShiftUpHorizontalLayersAfterEverythingLoaded = !
    0,
    splashContainerDiv = document.getElementById(
        "splash-container"),
    balloonDiv = document.getElementById(
        "balloon"),
    groundAndGrassContainer1Div =
    document.getElementById(
        "ground-and-grass-container-1"),
    elevation1Div = document.getElementById(
        "elevation-1"),
    elevation2Div = document.getElementById(
        "elevation-2"),
    ...
    sea1Div = document.getElementById(
        "sea-1"),
    seaFloorDiv = document.getElementById(
        "sea-floor"),
    seaFloorFrontObjectArray = new Array,
    seaFloorBackObjectArray = new Array,
    about1ContainerDiv = document.getElementById(
        "plants-container"),
    plantLine1Div = document.getElementById(
        "plant-line-1"),
    plantLine2Div = document.getElementById(
        "plant-line-2"),
    plantArray = new Array,
    plantTargetTopObjectArray = new Array;
plantTargetTopObjectArray.push(
    plantLine1Div, plantLine1Div,
    plantLine2Div, plantLine2Div);
var canAnimatePlantInformation,
    about2ContainerDiv = document.getElementById(
        "buildings-container"),
...
var buildingLegArray = new Array,
    ...
    buildingLegContainer1Div = document
    .getElementById(
        "building-leg-container-1"),
    buildingLegContainer2Div = document
    .getElementById(
        "building-leg-container-2"),
    buildingLegContainer3Div = document
    .getElementById(
        "building-leg-container-3"),
    buildingLegFrameArray = new Array,
    building1LegFrameDiv = document.getElementById(
        "building-1-leg-frame"),
    building2LegFrameDiv = document.getElementById(
        "building-2-leg-frame"),
    building3LegFrameDiv = document.getElementById(
        "building-3-leg-frame"),
    experience1ContainerDiv = document.getElementById(
        "experience-1-container"),
    experience2ContainerDiv = document.getElementById(
        "experience-2-container"),
    experience3ContainerDiv = document.getElementById(
    ...
    experienceTextContainerDistanceFromFloor =
    185,
    robotDiv = document.getElementById(
        "robot"),
    animateRobotHandsTimer,
    spinRobotHandsTimer,
    changeRobotHandsCounter = 0,
    robotHandLeftDiv = document.getElementById(
        "robot-hand-left"),
    robotHandRightDiv = document.getElementById(
        "robot-hand-right"),
    robotHandChildrenLength = $(
        robotHandLeftDiv).children().length,
    squidDiv = document.getElementById(
        "squid"),
    ...
    alienDiv = document.getElementById(
        "alien"),
    animateAlienHandsTimer,
    alienSteerDiv = document.getElementById(
        "alien-steer"),
    alienSteerAngle = 0,
    alienSteerPreviousAngle,
    ...
    piechartFoxnewsFrontDiv = document.getElementById(
    ...
    bubbleDiv = document.getElementById(
        "bubble"),
    bubbleTimer,
    shiftUpLayerHorizontalDistance,
    shiftUpLayerHorizontalTimer,
    ...
    blinkSeaAnimalsTimer,
    skill1ContainerDiv = document.getElementById(
        "skill-1-container"),
    fishArray = new Array,
...
numberOfFishInEachRowArray.push(5, 5, 4,
    4);
var skill2ContainerDiv = document.getElementById(
        "skill-2-container"),
    crabArray = new Array,
...
numberOfCrabInEachRowArray.push(5, 5, 4,
    3);
var skill3ContainerDiv = document.getElementById(
        "skill-3-container"),
...
numberOfTurtleInEachRowArray.push(3, 2,
    2, 2);
var pageVerticalPosition = 0,
    ...
    deltaPageVerticalPosition = 0,
    isRobbySwimming = !1,
    isRobbyJumping, isRobbyFalling,
    isRobbyBelowSeaLevel = !1,
    swimUpHeight, layersMovement,
    elevationArray = new Array,
    elevationNumberBelowRobby = null,
    robbyRightEdge, robbyLeftEdge,
    distanceBetweenRobbyAndBalloon =
    150,
    robbyMaxHorizontalDistance, counter =
    0,
    switcher = 1,
    robbyStaticFrame = 0,
    ...
    robbyStartFrame, robbyStopFrame,
    shiftRobbyFrameTimer,
    pageVerticalPositionWhenAnimateRobby1,
    pageVerticalPositionWhenAnimateRobby2,
    minimumVerticalDistanceToTriggerRobbySwimDownFrame =
    ...
    about3ContainerDiv = document.getElementById(
        "nba-container"),
    nbaPlayerDiv = document.getElementById(
        "nba-player"),
    nbaPlayerContainerDiv = document.getElementById(
        "nba-player-container"),
    nbaPlayerFrameDiv = document.getElementById(
        "nba-player-frame"),
    nbaRimContainerDiv = document.getElementById(
        "nba-rim-container"),
    nbaBallDiv = document.getElementById(
        "nba-ball"),
    canAnimateNbaInformation,
    nbaBoardsCounter,
    nbaBoardsAnimationTimer,
    nbaPlayerCounter, nbaPlayerTimer,
    nbaPlayerEyesDiv = document.getElementById(
        "nba-player-eyes"),
    blinkNbaPlayerTimer,
    contactContainerDiv = document.getElementById(
        "contact-container"),
    socialContainerDiv = document.getElementById(
        "social-container"),
    canAnimateSocialContainer,
    happyRobbyTimer, isRobbyHappy = !1,
    scrollOrSwipeTextContainer1Div =
    document.getElementById(
    ...
    contactConfirmationContainerArray =
    new Array,
    emailAddressDiv = document.getElementById(
        "email-address"),
    emailSubjectDiv = document.getElementById(
        "email-subject"),
    emailMessageDiv = document.getElementById(
        "email-message"),
    isContactConfirmationContainerVisible = !
    0,
    waterfall1Div = document.getElementById(
        "waterfall-1"),
    waterfall2Div = document.getElementById(
        "waterfall-2"),
    waterfallTimer, touchStartX = 0,
    touchCurrentX = 0,
    touchEndX = 0,
    fireworksContainerDiv = document.getElementById(
        "fireworks-container"),
    ...
    fireworkDotRadius = 5,
    fireworkCenterX, fireworkCenterY,
    fireworkOneRadiusDistance,
    fireworkOneRotationAngle,
    drawOneLayerOfFireworkTimer,
    canDrawManyFireworks = !0;
...
disableScrollOrSwipe(), $(window).on(
    "beforeunload",
    function() {
        $(window).scrollTop(0)
